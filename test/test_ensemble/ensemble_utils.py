...
from common.ensemble_building.abstract_ensemble import AbstractEnsemble
from common.ensemble_building.ensemble_builder import EnsembleBuilder


def constant_loss_fn(a, b):
    return 0.1


def log_loss_fn(labels, predictions):
    return log_loss(labels, predictions)


def roc_auc_loss_fn(labels, predictions):
    return 1 - roc_auc_score(labels, predictions)


def accuracy_loss_fn(labels, predictions):
...
class BackendMock(object):
    ...
    def load_targets_ensemble(self):
        with open(
            os.path.join(
                self.temporary_directory, ".auto-sklearn", "predictions_ensemble_true.npy"
            ),
            "rb",
        ) as fp:
